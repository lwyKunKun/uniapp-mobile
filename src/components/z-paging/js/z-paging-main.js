...
export default {
    ...
    watch: {
        ...
        totalData(newVal, oldVal) {
            ...
            newVal = [...newVal];
            if (this.loadingStatus === 2 && this.hideLoadingMoreWhenNoMoreByLimit > 0 && newVal.length) {
                this.showLoadingMore = newVal.length > this.hideLoadingMoreWhenNoMoreByLimit;
            } else if (this.loadingStatus === 2 && this.hideLoadingMoreWhenNoMoreAndInsideOfPaging && newVal.length) {
                this.$nextTick(() => {
    ...
    computed: {
        ...
        showEmpty() {
            const showEmpty = !this.refresherOnly &&
                !this.totalData.length &&
                (this.autoHideEmptyViewWhenLoading ? this.isAddedData : true) &&
                !this.hideEmptyView &&
                (this.autoHideEmptyViewWhenLoading ? !this.firstPageLoaded && !this.loading : true);
            return showEmpty;
        },
        tempLanguage() {
        ...
        nViewIs() {
            ...
            return finalNvueListIs === "scroller" || finalNvueListIs === "view" ? "view" : finalNvueListIs === "waterfall" ? "header" : "cell";
        },
    ...
    methods: {
        ...
        _shouldShowLoading(type) {
            ...
            if (
                ((!this.showLoadingMoreWhenReload || this.isUserPullDown || this.loadingStatus !== 1) && !this.showLoadingMore) ||
                (!this.loadingMoreEnabled && (!this.showLoadingMoreWhenReload || this.isUserPullDown || this.loadingStatus !== 1)) ||
                this.refresherOnly
            ) {
                return false;
            }
            ...
            if (type === "loadingMoreDefault") {
                ...
                const res = this.showDefaultLoadingMoreText && !(this.loadingStatus === 2 && !this.showLoadingMoreNoMoreView);
                return res;
            }
            return false;
        },
        ...
        _getRefresherTouchDisabled() {
            ...
            const res =
                ...
                (this.usePageScroll && this.useCustomRefresher && this.pageScrollTop > 10) ||
                (!(this.usePageScroll && this.useCustomRefresher) && checkOldScrollTop);
            return res;
        },
        //本地分页请求
