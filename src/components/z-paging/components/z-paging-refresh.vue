  ...
  <view style="height: 100%">
    <view
      :class="[
        ...
        { 'zp-custom-refresher-container-padding': showRefresherUpdateTime },
      ]"
      style="height: 100%"
    >
      <view class="zp-custom-refresher-left">
        <image
          v-if="refresherStatus !== 2"
          :class="refresherLeftImageClass"
          :style="[
            {
              width: showRefresherUpdateTime ? '36rpx' : '30rpx',
              height: showRefresherUpdateTime ? '36rpx' : '30rpx',
              'margin-right': showRefresherUpdateTime ? '20rpx' : '8rpx',
            },
          ]"
        ...
        <image
          v-else
          class="
            zp-loading-more-line-loading-image zp-custom-refresher-left-image
          "
          :style="[
            {
              width: showRefresherUpdateTime ? '36rpx' : '30rpx',
          ...
          :src="
            defaultThemeStyle === 'white' ? base64FlowerWhite : base64Flower
          "
        >
        </image>
        <!-- #endif -->
        ...
        <view
          v-else
          :style="[
            { 'margin-right': showRefresherUpdateTime ? '18rpx' : '12rpx' },
          ]"
        >
          <loading-indicator
            :class="
            ...
            "
            :style="[
              { color: defaultThemeStyle === 'white' ? 'white' : '#777777' },
            ]"
            :animating="true"
          >
          </loading-indicator>
      ...
      <view class="zp-custom-refresher-right">
        <text
          class="zp-custom-refresher-right-text"
          :style="[refresherRightTextStyle]"
          >{{ refresherStatusTextMap[refresherStatus] || refresherDefaultText }}
        </text>
        <text
          class="
            zp-custom-refresher-right-text zp-custom-refresher-right-time-text
          "
          :style="[refresherRightTextStyle]"
          v-if="showRefresherUpdateTime && refresherTimeText.length"
          >{{ refresherTimeText }}
        </text>
      </view>
    </view>
...
.zp-custom-refresher-arrow-top {
  /* #ifndef APP-NVUE */
  animation: refresher-arrow-top 0.2s linear;
  -webkit-animation: refresher-arrow-top 0.2s linear;
...
.zp-custom-refresher-arrow-down {
  /* #ifndef APP-NVUE */
  animation: refresher-arrow-down 0.2s linear;
  -webkit-animation: refresher-arrow-down 0.2s linear;
  animation-fill-mode: forwards;
  -webkit-animation-fill-mode: forwards;
  /* #endif */
  /* #ifdef APP-NVUE */
