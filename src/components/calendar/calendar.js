...
var calendarWidget = {
    ...
    renderCalendar: function() {
        ...
        for (var i = 1; i <= totalDay; i++) {
            ...
            for (var z = 0; z < calendarWidget.config.mark.length; z++) {
                //判断是否为标记点
                if (tempDay == calendarWidget.config.mark[z].time.trim()) {
                    var tempTextStr =
                        calendarWidget.config.formatText == undefined ? calendarWidget.config.mark[z].text : calendarWidget.config.formatText(calendarWidget.config.mark[z].text);
                    textTemp = "<div class='dayValue'>" + fontText;
                    if (calendarWidget.config.mark[z].markPoint != false)
                    // 是否显示点
                        textTemp += calendarWidget.config.mark[z].pointText ?
                        '<span class="calendar-point">' + calendarWidget.config.mark[z].pointText + "</span>" :
                        '<span class="calendar-point">●</span>';
                    if (tempTextStr != undefined && tempTextStr != "")
                    // 是否显示文字
            ...
            if (calendarWidget.range.start && calendarWidget.range.end) {
                if (calendarWidget.range.start <= tempCurrTime && tempCurrTime <= calendarWidget.range.end) calendar_range = " calendar-range ";
            }

            if (tempDay == currentDateTemp) {
                //当天
                htmlStr +=
                    '<div class="calendar-col aligncanter currentDays calendar-today ' + calendar_range + '" data-day="' + i + '" data-date="' + tempDay + '">' + textTemp + "</div>";
            } else {
                //其他
        ...
        var content =
            "<div class='calendar-Time-header under_line'><a class='calendar-lastMonth'>" +
            calendarWidget.config.lastText +
            "</a><a  class='calendar-TimeH'>" +
            textStr +
            "<a><a class='calendar-nextMonth'>" +
            calendarWidget.config.nextText +
            "</a> <a class='calendar-backToToday'>回到今天</a></div><div class='ClearBoth'></div>" +
        ...
        if (calendarWidget.config.mounted)
        //初始化完成
    ...
    calendarTransform: function(x, time) {
        //日历滑动动画
    ...
    change: function() {
        ...
        if (calendarWidget.config.change) calendarWidget.config.change(tempd, markPoint);
    },
    ...
    setMark: function(data, cover) {
        ...
        if (data && data.length > 0) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                var tempTextStr = calendarWidget.config.formatText == undefined ? data[i].text : calendarWidget.config.formatText(data[i].text);
    ...
    setRange: function(startDate, endDate) {
        ...
        } else console.error("setRange : error param");
    },
    clearRange: function() {
        //去掉范围标记
        calendarWidget.range.start = undefined;
