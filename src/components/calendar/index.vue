...
<template>
  <view>
    <view class="calendar-Time-header under_line">
      <view class="calendar-header-left"></view>
      <view class="calendar-header-middle">
        <text
          class="calendar-lastMonth"
          @tap="subMonth"
          v-bind:class="{ 'calendar-btn-disabled': lastDisabled }"
          >{{ lastText }}</text
        >
        <text class="calendar-TimeH">{{ title_time }}</text>
        <text
          class="calendar-nextMonth"
          @tap="addMonth"
          v-bind:class="{ 'calendar-btn-disabled': nextDisabled }"
          >{{ nextText }}</text
        >
      </view>
        ...
        >
          <view class="dayValue">
            ...
            <text
              ...
              v-if="!item.isIcon"
              >{{ item.pointText == '' ? '●' : item.pointText }}</text
            >
          ...
          for (let z = 0; z < that.mark.length; z++) { //判断是否为标记点
            ...
            if (tempDay.getTime() == tempDayMark.getTime()) {
              ...
              if (that.mark[z].markPoint != false) { // 是否显示点
                tempObj.isIcon = that.mark[z].isIcon ? that.mark[z].isIcon : undefined;
                tempObj.icon = that.mark[z].icon ? that.mark[z].icon : undefined;
