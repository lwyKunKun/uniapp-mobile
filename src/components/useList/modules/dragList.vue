  <!-- 组件思路：利用touch事件返回坐标值，赋值滑动视图，得到滑动视图跟随鼠标的效果，视觉上达到拖拽。-->
<template>
  <view class="">
    <!-- 可移动区域容器 -->
    <movable-area class="movarea" ref="areaBox" id="areaBox">
      ...
      <view class="appList">
        ...
        >
          <view class="appIcon">
            <u-icon
              :name="appItem.appIcon"
              custom-prefix="custom-icon"
            ></u-icon>
          </view>
          <text class="appName">{{ appItem.appName }}</text>
          <view
            class="appIcon cuIcon-roundclosefill"
            :class="deleteAppID === appItem.appId && showDelete ? '' : 'hide'"
            @tap="deleteAppItem(index)"
          >
            <u-icon name="shanchu4" custom-prefix="custom-icon"></u-icon>
          </view>
        </view>
        <view class="app-li" @click="addAppItem">
          <view class="appIcon">
  ...
  methods: {
    ...
    confirm () {
      let appItem = {
        ...
        appIcon: "gonggao",
        appName: this.$refs.addAppInput.form.applyName,
        appLink: ""
      };
